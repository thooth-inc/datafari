		...
		try {
			...
			try {
				server = SolrServers // Select the right core
						.getSolrServer(Core.PROMOLINK, protocol);
	...
	protected void doPost(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (request.getParameter("title") != null && request.getParameter("keyword") != null
					&& request.getParameter("contentPromoLink") != null) { // If
																			// it's
																			// an
																			// edit
																			// or
																			// an
																			// add
				final Object key = request.getParameter("keyword"), title = request.getParameter("title"),
						value = request.getParameter("contentPromoLink"), oldKey = request.getParameter("oldKey");
				...
				doc = new SolrInputDocument();
				try {
					doc.addField("keyword", key); // add the keyword to the
													// Solrdoc
					doc.addField("title", title); // add the title to the
													// Solrdoc
					doc.addField("content", value); // add the value to the
													// Solrdoc
					...
					if (request.getParameter("oldKey") != null) { // If it's an
						...
						if (request.getParameter("oldKey") != request.getParameter("keyword"))
							server.deleteById(oldKey.toString()); // Delete the
																	// previous
																	// promolink
																	// on the
																	// keyword
